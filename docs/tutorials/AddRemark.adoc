...
This command allows users of the AddressBook application to add optional remarks to people in their protein book and edit it if required.
...
Let's start by creating a new `RemarkCommand` class in the `src/main/java/seedu/protein/logic/command` directory.
...
[source, java]
----
package seedu.protein.logic.commands;

import seedu.protein.model.Model;

/**
 * Changes the remark of an existing food in the protein book.
 */
...
[source, java]
----
import static seedu.protein.commons.util.CollectionUtil.requireAllNonNull;
//...
public class RemarkCommand extends Command {
    //...
...
Create a `RemarkCommandParser` class in the `seedu.protein.logic.parser` package.
...
Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the food's name).
That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a food.

=== Add a new `Remark` class

Create a new `Remark` in `seedu.protein.model.food`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
to link:https://github.com/nus-cs2103-AY1920S1/addressbook-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-0c6b6abcfac8c205e075294f25e851fe[`seedu.protein.ui.PersonCard`].
...
Not doing so will cause AddressBook to default to an empty protein book!
