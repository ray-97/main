...
In this tutorial, we'll do exactly just that and remove the `protein` field from `Person`.
...
The `protein` field in `Person` is actually an instance of the `seedu.protein.model.food.Address` class.
...
. Safe delete the field `protein` in `EditPersonDescriptor`
...
. Remove the usages of `protein` and select `Do refactor` when you are done.
+
TIP: Removing usages may result in errors.
Exercise discretion and fix them.
For example, removing the `protein` field from the `Person` class will require you to modify its constructor.
...
You can find them by searching for instances of the word `protein` in your code (`Edit` > `Find` > `Find in path`).
...
For example, there is a `$protein` in each `PersonCard` that has not been removed nor identified.

image::$protein.png[width=1090px, height=890px]
...
While keeping the `protein` field in the json files does not cause the tests to fail, it is not good practice to let cruft from old features accumulate.
...
```
You can go through each individual `json` file and manually remove the `protein` field.
