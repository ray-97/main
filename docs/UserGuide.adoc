...
Calgo's graphical user interface (GUI) is designed to be sleek and compact, allowing you to use Calgo on nearly entirely
one window and also take in information at a glance. However, that might be a little overwhelming at the start. Hence,
this section aims to give a rundown on the GUI's various parts and ease you into using the App, starting from the
top of the GUI.

There are eight parts to the GUI, as follows: `Menu Bar`, `Command Box`, `Result Display`, `Daily Goal Display`, `Consumption Record`,
 `Graph Display`, `Food Record` and `Status Bar`. They are labelled accordingly in the above image from 1 to 8. There is
a ninth part, the `Help Window`, that is a separate pop up - only appearing when required for your convenience.

=== `Menu Bar`
The bar right at the top of Calgo's GUI. It only has two button, `File` and `Help`. The buttons allow
...
`exit` command, and likewise accessing the command guides can be done using the `help` command.

=== `Command Box`
Calgo listens to your requests and processes them. The `Command Box` is where you type commands into.

[[TIP]]
Should you mistype a command, Calgo will prompt you to use the right format! You can easily edit the errornous command
inside the `Command Box` without needing to retype it entirely.

=== `Result Display`
This is where Calgo displays textual updates as a result of processing your inputted commands. Although some commands
update elsewhere on the GUI, the `Result Display` will still show a response so you know for certain when your
commands have been correctly processed.

=== `Daily Goal Consumption`
This section is helpfully labelled as "Your Goal Information" on the GUI, and consists of two parts. The left box
showcases your target daily calorie goal (after you've set it using the `goal` command). The right box showcases
...
one day's consumption, using the `stomach` command will let you view other days' records.

It has three subsections: The name of the `Food` entry, the number of portions of that `Food` you consumed that day,
and its average rating as set by you, color-coded from red to green for low to high rating.

=== `Graph Display`
This handy graph shows a summary of your past seven day's total calorie consumption. It displays starting from the date
of the current `Consumption Record` and then counts the six days before it.

eg. `Consumption Record` currently shows 2020-04-12. `Graph Display` will thus show the daily calories consumed from
2020-04-06 to 2020-04-12.

The `Graph Display` is a line graph for clear data representation, and to ensure you aren't overly confused by
unnecessary information on it.
...
=== `Status Bar`
All the way at the bottom, this bar shows where data saved via the `export` function is saved to in your computer. Simply
navigate via a file explorer or copy paste that path into the file directory to view any saved exported data.

[[Commands]]
== Commands

In this section, specific information about each command will be explained, such as what the command can do for you, how is it formatted and visual examples of their usage.

But before that, do take note of the following information, which applies to all Calgo commands:
====
*Command Format*

* Words in `UPPER_CASE` are parameters that are to be provided by you.
...
** *Example:* If the command specifies `n/NAME p/PROTEINS`, `p/PROTEINS n/NAME` gives the same functionality.
* Items with `…`​ after them can be used multiple times (including zero times).
** *Example:* `[t/TAG]...` implies it can be left empty, or used as `t/favourite`, `t/favourite t/green` or with more than 2 tags.
====

In addition, this is a useful shortcut that may apply to you:

TIP: As you key in the name of the `Food` for any of the following commands: `update`, `delete`, or `nom`, Calgo shows you
`Food` with similar names that exists in the `Food Record` in the `Result Display` component of the GUI.
Never make spelling mistakes again!

Now that you know the general command information, refer to the following subsections for specific details of Calgo commands.

=== Clearing all entries : `clear`
(by Vineeth)

If you want to clear all the `Food` entries currently stored in your `Food Record`, this command will be extremely useful to you.

Format: `clear` +

Example: Suppose that you want to drastically improve your diet. You decide to take a look at all of your `Food` items in the `Food Record` currently but none of them fit into your new diet.
So instead of deleting each `Food` item one by one, you decide to `clear` them. This is what you need to do:

image::ClearCommandBefore.png[width=]

First type `clear` in the `Command Box` and press kbd:[enter], as shown in the above diagram.

image::ClearCommandAfter.png[width=]

Upon entering the command, all your `Food` items in the  `Food Record` will be deleted, resulting in an empty `Food Record`, as shown above.

Note that your `Consumption Record` data is not affected from executing the `clear` command as shown by the unchanged Daily List and graph in the above diagram.

To cater to fast typists that may make occasional typing mistakes, Calgo will still execute `clear` even if there are other parameters or words that are typed after `clear`. +
