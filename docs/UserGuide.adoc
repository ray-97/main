...
currently in the `Food Record` but none of them fit into your new diet.
So instead of deleting each `Food` entry one by one, you decide to `clear` them. This is what you need to do:

image::ClearCommandBefore.png[width=]

First, type `clear` and press kbd:[enter], as shown in the above diagram.
...
Note that your `Consumption Record` and `Graph Display` data is not affected from executing the `clear` command as shown by the above diagram.

To cater to fast typists who are more likely to make typing mistakes, Calgo will still execute `clear` even if there are
...
You can type `help nom`, then press kbd:[enter].

image::HelpCommandAfter.png

As you can see, a `Help Window` is created, showing a guide on how to use the `nom` command.

=== `exit`: Closing Calgo

If you want to exit the App, you can use the `exit` command. All your data is automatically saved upon
exiting Calgo.
...
reference sheet to include information outside of the App. +
[NOTE]
...
Say you wish to export the current `Food Record` so that you can print a copy for reference while cooking. Here's how you can do it:
...
to `Food` entries containing them in the specified parameter of name or tag. +
...
You should type `find t/swe` as input, and then press kbd:[enter]. +
...
Please note that the search is case-insensitive, an example being the `Sweet` tag of `Bandung`. Moreover,
...
After keeping track of your meals, don't you want to go to the next step and use your past consumption patterns
to learn how to improve your eating habits? Won't it be convenient if you can automatically generate tips on how to improve?
That is exactly what `report` command can help you with!

The `report` command analyses your `Consumption Record` to generate
helpful insights for improving your diet.

[note]

Given a date, the command generates a document that contains relevant insights related to your food consumption
pattern on that day. This document is stored in the `data/reports` folder. The name of this document is "report_yyyy_mm_dd.txt". For instance, if you generate a report on 27th of April 2020, the generated document is "report_2020-04-27".

Refer to the list below for the information generated by the `report` command.
****

* All `Foods` consumed on the given date. For each `Food`, the following information will be included:
** The name of the `Food` item.
** Quantity consumed, for instance 3 portions.
...
Let's suppose you had a buffet on 27th of March 2020 and you managed to `nom` lots of exciting `Food` into your `Consumption Record`. It is now the end of the day and you want to look back and find out how you fared. This is what you can do:

image here

First, type `report d/27-03-2020` and press kbd:[enter], as shown above.

Explain step by step what each section refers to.
...
What is the point of tracking your meals if you do not have an objective benchmark? Don't you want to see noticeable progress in your eating habits?

The `goal` command will help you to set a numerical goal for yourself, which is essentially the number of calories you want
to consume each day. Note that this goal must be a positive whole number.

Your goal will also be used to provide helpful insights regarding your consumption patterns. Hyperlink to report. +
...
Let's suppose you are a young adult who is trying to lose weight to fit into those tight jeans. You want to lose weight so you google around for the ideal number of calories you should take to meet your goal. A credible online source states it should be 1900 calories. This is how you update your goal into Calgo:

Screenshot

Type in `goal 1900` and press kbd:[enter] +

Screenshot

[note]
* After setting a daily calorie goal, you can still update it subsequently!
* The goal that you set will be used to generate insights in the `report` command. (Add hyperlink to report)
...
=== `update` : Updating the current `Food Record`
(by Zhang Xian)
