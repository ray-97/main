...
To cater to fast typists that may make occasional typing mistakes, Calgo will still execute `clear` even if there are other words that are typed after `clear`. +
Example: `clear you are awesome Calgo` will still work as Calgo intelligently ignores the remaining words. However, if there is no whitespace typed after `clear`, Calgo will not be able to understand the entered command. Refer to the diagram below for a visualisation.

image::ClearCommandError.png[width=]
