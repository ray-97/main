...
pattern on that day. This document is stored in the `data/reports` folder. The name of this document
is yyyy-mm-dd_report.txt. For instance, if you generate a report on 27th of April 2020, the generated document is 2020-04-27_report.txt.
...
** Number of calories consumed from each `Food` entry.
...
Let's suppose you had a buffet on 27th of March 2020 and you managed to `nom` lots of exciting `Food` into your
...
Type `report d/27-03-2020` and press kbd:[enter], as shown above.
...
The `list` command resets the display accordingly to show all entries in the `Food Record`. These will be neatly sorted,
...
You will then see a result message displayed, and the `Food Record` will now show all `Food` entries.
...
Thus, to motivate you to put in consistent effort, you can set daily caloric goals for yourself using the `goal` command.
...
Let's suppose you are a young adult who is trying to lose weight to fit into those tight jeans. You want to lose weight
so you search online for the ideal number of calories you should take to meet your goal. A credible online source states
it should be 1900 calories. You can update your daily goal by doing the following:
...
*Goal*: Daily caloric goal set by the user.
...
|Stomach|Lists all `Food` in a particular day's `Consumption Record`.|`stomach [d/DATE]`|`stomach d/2020-04-20`
|Vomit|Deletes a specific `Food` from the `Consumption Record`.|`vomit num/INDEX_OF_FOOD [d/DATE] [portion/PORTION]`
    |`vomit num/chicken d/2020-03-04 portion/3` +
    *OR* +
    `vomit num/chicken` +
    *OR* +
    `vomit n/chicken d/ portion/`
.4+|Generate Document|Report|Generates the report for a given date.|`report d/DATE in YYYY-MM-DD`|`report d/2020-04-20`
