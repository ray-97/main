...
Format: `find [n/NAME] [cal/CALORIES] [p/PROTEINS] [c/CARBOHYDRATES] [f/FATS] [t/TAG]` (choose only 1 parameter)
...
|=======================================================================
|Command Category|Command Name|Usage|Format
.3+|Navigate app|Exit|Exits the app|`exit`
|Goal|Sets your daily caloric goal.|`goal GOAL`
|Help|Displays a guide for all commands. Can also display only commands containing the given command word.|`help COMMAND_WORD`
.3+|Edit Food Record|Clear|Clears all `food` entries from the Food Record.|`clear`
|Delete|Deletes the specified `Food` from the `Food Record`.|`delete`
|Update|Updates a `Food` in `Food Record`.|`update n/NAME cal/CALORIES p/PROTEINS c/CARBOHYDRATES f/FATS [t/TAGS]`
.2+|Navigate Food Record|Find|Searches for and displays a particular `Food`.|`find [n/NAME] [cal/CALORIES] [p/PROTEINS] [c/CARBOHYDRATES] [f/FATS] [t/TAG]` (choose only 1 parameter)
...
|Stomach|Lists all `Food` in a particular day's `Daily List`.|`stomach [d/DATE]`
|Vomit|Deletes a specific `Food` from the `Daily List`.|`vomit num/INDEX_OF_FOOD [d/DATE] [portion/PORTION]`
|Generate Report|Report|Generate the report for a given date.|`report d/DATE`
|=======================================================================
...
* *goal* : `goal GOAL` +
e.g `goal 2000`
* *Report* : `report Date in YYYY-MM-DD`
