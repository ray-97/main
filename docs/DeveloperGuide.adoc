...
.  The product should have minimal network usage.
. It is expected that users will find out about the
...
[[food]] Food::
A `Food` item entered by the user represents a real life food.
`Food` contains nutritional values such as `Calorie` s, number of grams of `Protein` s, `Carbohydrate` s and `Fat` s.
It can also contains a series of `Tag` s.
