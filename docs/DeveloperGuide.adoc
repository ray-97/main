...
=== Food Consumption Manage
In Calgo, you will find that there is a date associated with each list of `ConsumedFood`.

* The UI's `StackPane` takes in a `FilteredList` to get `ConsumedFood` items to display.
* Whenever a consumption command is used, the `FilteredList` will be repopulated with the food from `UniqueDateToLogMap`
according to the `LocalDate` specified.

==== Implementation
The `Nom`, `Vomit` and `Stomach` commands are facilitated by the `FoodRecord`.  +
`FoodRecord` contains a `UniqueDateToLogMap`, which maps a `LocalDate` to a `DailyFoodLog` which contains a `LinkedHashMap`
of the `Food` that user consumed/removed on that date.
