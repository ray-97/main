...
[discrete]
=== Use case: UC1 - Find Food item by name
*System:* Calgo +
*Actor:* User +
*MSS:*

1. User uses the find command along with a partial/complete string which he wishes to match.
2. Calgo displays a drop down list of suggestions while user is entering the name of the food.
3. User decides on the input and presses enter.
4. Calgo outputs a list of Food items that has name matching the string
+
Use case ends.
...
[none]
3a. Invalid input format, such as wrong prefixes used. +
&ensp;&ensp;3a1. Calgo displays a message warning about wrong format. +
&ensp;&ensp;Use case ends

4a. Food item does not exist. +
&ensp;&ensp;4a1. Calgo displays a message indicating that no such Food item exists. +
&ensp;&ensp;Use case ends

[discrete]
=== Use case: UC2 - Find Food item by nutritional value
*System:* Calgo +
*Actor:* User +
*MSS*
...
[none]
2a. The Food record is empty. +
&ensp;&ensp;2a1. A message indicating that no such Food item exists is shown. +
&ensp;&ensp;Use case ends.
