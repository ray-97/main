...
- In the above sequence diagram, two possibilities are shown. If the user provides a `command_word`, `setFilteredGuide`
...
[NOTE]
`Prefix` here indicates which `Food` attribute we are interested in. Categorical search finds `Food` objects with matching values representing one of the nutritional categories (`Calorie`, `Protein`, `Carbohydrate`, or `Fat`). Meanwhile, substring search finds matches of a substring in any part of the the `Name` or in any of the `Tag` objects belonging to the `Food`.
...
The above commands rely on the `FindCommand` and `ListCommand` classes respectively. Objects of both classes use a `Predicate<Food>` object to filter through the current `Food` objects, which will be displayed in the GUI's `Food Record` should they evaluate these predicates to be true.
...
- Can use regular expressions, which are powerful and suitable for our purpose.
** Cons:
...
- Need to implement searching via multiple types of `Food` attributes and hence more dependencies.
...
* Thereafter, `UniqueFoodList` sorts the `Food` objects whenever they are added or edited in the `Model`.
...
Step 3: Once `UniqueFoodList` is constructed, we introduce the initialising data into it via the `setFood` method. This calls the `sortInternalList` method, which sorts the `ObservableList<Food>` contained in `UniqueFoodList` according to the specified lexicographical order (defined in the `Food` class).


The sequence diagram below (which is a reference frame omitting irrelevant `update` command details) describes the lexicographical sorting process when `Food` objects are added or edited using the `update` command:
...
* If a `Food` object with an equivalent `Name` (as what the user inputs) already exists in the `UniqueFoodList` object:
- Step 1: The `UpdateCommand` object calls the `getExistingFood` method of the `ModelManager` for the user-entered `Food`, which then calls that of `FoodRecord`, and subsequently that of `UniqueFoodList` to eventually obtain the existing `Food` object with an equivalent `Name`. +
- Step 2: Using the same sequence of classes, we call the respective `setFood` methods, eventually replacing the existing `Food` object with the desired one and arriving at the `sortInternalList` method of `UniqueFoodList`. +
...
* Otherwise, the user-entered `Food` is an entirely new `Food` object without a `Name`-equivalent `Food` that exists in the `UniqueFoodList`:
- Step 1: Using the same sequence of classes as the former case, we call the `addFood` and `add` methods of the respective classes, eventually adding the `Food` object and arriving at the `sortInternalList` method of `UniqueFoodList`. +
