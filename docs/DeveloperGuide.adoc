...
gives you a way to browse a list of `Food` within the `ConsumptionRecord` at a different date.
...
// inserts class diagram for illustration

// -> continue here!
This section shows you how `nom` command works.
...
* If the user inputs a `Food` object that already has an existing counterpart with an equivalent `Name` in the `UniqueFoodList`:
