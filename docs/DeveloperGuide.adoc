...
=== Lexicographical-sorted `list`

With a large number of `Food` objects in the App, it will be useful to see all entries sorted in lexicographic order. +

Here, the `list` command displays all the current `Food` entries in the `Model`.
This is often more useful after a `find` command, which narrows down the displayed entries after a search.
While it may seem that the `list` command is responsible for keeping the `Food` entries sorted, that is not the case; instead,
the entries are sorted whenever they are added or edited.

==== Implementation

Sorting `Food` objects by lexicographic order is actually done by the `UniqueFoodList`.
...
* Existing `Food` objects are arranged in lexicographic order by `Name`.
